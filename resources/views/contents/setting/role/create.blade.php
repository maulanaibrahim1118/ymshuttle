...
<div class="card card-stats card-round">
            ...
            <div class="accordion-item mx-2">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed p-0" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                        <h5 class="card-title mb-0"><i class="fas fa-plus me-3"></i>Add Role</h5>
                    </button>
                </h2>
            </div>
        </div>
    ...
    <div id="collapseOne" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
        <div class="accordion-body">
            <div class="card-body">
                <form id="addForm" class="row g-3 p-2" action="{{ route('roles.store') }}" method="POST">
                    @csrf
                    <input type="hidden" id="id" name="id" value="{{ encrypt(0) }}">
                    <div class="form-group unique-name @error('name') has-error @enderror col-md-3">
                        <label class="form-label">Role Name*</label>
                        <input type="text" name="name" class="form-control alert-warning text-uppercase" id="name" value="{{ old('name') }}" required>
                        <label id="name-taken" style="display: none;">Role Name exists!</label>
                    </div>

                    <div class="form-group col-md-2">
                        <label class="form-label">Guard Name</label>
                        <input type="text" name="guard_name" class="form-control alert-warning text-uppercase" id="guard_name" value="web" maxlength="10" readonly>
                    </div>
